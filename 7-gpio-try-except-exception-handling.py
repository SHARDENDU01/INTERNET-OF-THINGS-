...
try:
	while 1:
		if NET.input(24)==0:			#WHEN THE SWITCH (24) IS NOT PRESSED i.e, (24)==0 then,THE VOLTAGE AT THE TERMINAL IS HIGH i.e, IN ELSE CASE WHEN Vout=3.3V (HIGH)
			...
			NET.output(22,1)
			NET.output(27,1)
			print "PIN 24  PRESSED (INPUT IS LOW) && OUTPUT IS HAVING 3.3V!!!!"
		elif NET.input(23)==0 and NET.input(24)==1:   #WHEN PIN 23 IS NOT PRESSED i.e,(23)==0 AND PIN 24 IS PRESSED i.e, (24)==1 
			NET.output(17,1)
			NET.output(22,0)
			NET.output(27,1)
			print "PIN 24(SWITCH) IS NOT PRESSED AND 23(SWITCH) IS PRESSED && OUTPUT IS HAVING PARTIAL VALUES"
		else:
			NET.output(17,0)				#IN CASE WHEN 23 IS ON AND WHATEVER 24 IS ON or OFF THIS CASE WILL RUN
			NET.output(22,0)				#also when 23 and 24 are not pressed
			NET.output(27,0)
			print "NO PIN IS PRESSED(INPUT IS HIGH) && OUTPUT IS HAVING 0 V "

		time.sleep(1)
except KeyboardInterrupt:
	NET.output(17,0)			#SETTING ALL THE OUTPUT PINS IN OFF POSITION AT THE END OF PROGRAM ON PRESSING CTRL+C KEYBOARD INTERRUPT
	NET.output(22,0)
	NET.output(27,0)
	print " Someone has pressed CTRL + C key on keyboard hence the program execution get stops.Thank you see you soon!!!!! "
